...
void MMJODB::on_actionOpenDatabase_triggered()
{
    ...
    dialog.setNameFilter(tr(STR_DIALOG_OPEN_DB_FILTER));
    dialog.setAcceptMode(QFileDialog::AcceptMode::AcceptOpen);
    dialog.setWindowTitle(tr(STR_DIALOG_OPEN_DB_TITLE));
    ...
    if (dialog.exec())
    {
        ...
        if (filenames.size() != 1)
        {
            m_emsg.showMessage(tr(STR_ERR_MSG_INVALID_SELECTION), tr(STR_INT_ERR_MSG_TYPE_INVALID_SELECTION));
        }
        else
        {
...
void MMJODB::on_actionNewDatabase_triggered()
{
    ...
    dialog.setWindowTitle(tr(STR_DIALOG_NEW_DB_TITLE));
